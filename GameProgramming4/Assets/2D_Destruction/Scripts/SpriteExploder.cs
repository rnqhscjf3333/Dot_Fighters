...
public static class SpriteExploder {
    ...
    private static GameObject generateTriangularPiece(GameObject source, List<Vector2> tri, Vector2 origVelocity, Vector3 origScale, Quaternion origRotation, Material mat)
    {
        ...
        // testing
        /*int fragmentFileNameCount = 1;
        string meshFilePath = "Assets/2D_Destruction/Prefab/mesh";
        string meshFilePathEnd = ".mesh";
        string pathCombine = "";
        while (true)
        {
            pathCombine = meshFilePath + fragmentFileNameCount + meshFilePathEnd;
            if (System.IO.File.Exists(pathCombine))
            {
                fragmentFileNameCount++;
            }
            else
            {
                AssetDatabase.CreateAsset(uMesh, meshFilePath + fragmentFileNameCount + meshFilePathEnd);
